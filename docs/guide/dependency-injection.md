...
The Tokamak repository includes several examples demonstrating different DI patterns:

### Basic DI with Services (`examples/hello`)

Shows simple dependency injection with a configuration and greeting service:
...
### Multi-Module DI (`examples/hello_app`)

Demonstrates organizing dependencies into separate modules:

- **ConfigModule** - Application configuration
- **ServicesModule** - Shared services (Logger, Metrics)
- **WebModule** - Web routes and handlers

Each module can inject dependencies from other modules, and use init hooks for setup.

### Service Layer with Dependencies (`examples/blog`)

Shows a complete service-oriented architecture:

- **Cache** - In-memory caching service with TTL
- **Validator** - Input validation service
- **BlogService** - Business logic that depends on both Cache and Validator

```zig
const BlogService = struct {
...
The Blog example demonstrates:
- Service composition (BlogService uses Cache and Validator)
- Multi-module organization
- Init hooks for startup logging
- Cross-cutting concerns (caching, validation)

Check the `examples/` directory in the repository for complete, runnable code.
