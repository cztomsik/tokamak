...
pub fn expectJson(arg: anytype, expected: []const u8) !void {
    try expectFmt(std.json.fmt(arg, .{ .whitespace = .indent_2 }), expected);
}
...
pub fn expectTable(items: anytype, comptime expected: []const u8) !void {
    ...
    const cols = comptime blk: {
        var cols: [std.mem.countScalar(u8, header, '|') - 1]Col = undefined;
